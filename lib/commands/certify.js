...
function* runCommand(context, heroku) {
    ...
    cli.log('Got next domains of the app:', appDomains.map(function(d) { return d.hostname; }).join(', '));
    debug("Detailed information about domains: %o", appDomains);

    var domainForChallenge = appDomains.find(function(d) { return d.kind === 'custom'; }).hostname;
    ...
    var le = letsEncrypt.create({
        server: 'staging',
        challenges: {'http-01': leChallenge, 'tls-sni-01': leChallengeNull},
        challengeType: 'http-01',
        debug: false
    });

    cli.log("Seeking a certificate in the local storage...");
    var certificate = yield le.check({domains: [domainForChallenge]});
    if (!certificate) {
